...
public class Test {
	...
	static final String query = "var director = g.V(\"<m.06pj8>\").Out(\"<type.object.name>\").ToArray()[0]" + ENTER//
			...
			+ "   var genre_list = \"\"" + ENTER + "   _.each(genre, function(gen){" + ENTER//
	...
	static final String greNoSub = "g.V().Out(\"<start>\").Out(\"<actor>\").All()";
	/** 
	 * {@link #valueToSub}
	 * <pre>
...
//			res = Query.queryByGremlin(host, 64211, valueToSub);
	...
	static final String gre = "g.V(\"<jiumao>\").Out(\"<start>\").Out(\"<actor>\").All()";
